...
def main(options,args):
	if (options.filename == False) and (options.text ==False) :
		...
		return 
	if (options.key==False) or len(options.key)<8 or len(options.key)>8:
		print("key cannot be empty and key length should be 8")
		return 
	if options.encrypt:
...
parser.add_option("-k", "--key",dest="key",default=False,help="enter the key here")
